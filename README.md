--[[ ASTA REMAKE – Full Hub (Simplificado) Speed Booster (LinearVelocity) integrado ao Carry Speed e Lagger. Botões móveis individuais + fundo personalizado. Botão de Lock na barra superior. Reset aprimorado (substitui Insta Reset e Medusa Reset). Bypass TP + Anti Die. Drop com ApplyImpulse. ANTI-KICK ROBUSTO (bloqueia qualquer velocidade enquanto pet estiver equipado). ANTI-RAGDOLL SIMPLES (forceReset com cooldown). BAT V2 integrado (opcional). ESP (quadrado + linha preta) – ativável via GUI. ]]
...
-- ============================================================
-- RESET (substitui Insta Reset e Medusa Reset) – MAIS RÁPIDO
-- ============================================================
...
function startBatV2()
    ...
    local hum = LP.Character and LP.Character:FindFirstChildOfClass("Humanoid")
    if hum then batV2Rot = hum.AutoRotate; hum.AutoRotate = false end
    batV2Enabled = true
...
-- ============================================================
-- MOBILE BUTTONS – INDIVIDUAIS ARRASTÁVEIS
-- ============================================================
...
task.spawn(function()
    while true do task.wait(0.1);updateAutoSwitchSpeed() end
end)
...
local MOVE_KEYS={[Enum.KeyCode.W]=true,[Enum.KeyCode.A]=true,[Enum.KeyCode.S]=true,[Enum.KeyCode.D]=true,[Enum.KeyCode.Up]=true,[Enum.KeyCode.Down]=true,[Enum.KeyCode.Left]=true,[Enum.KeyCode.Right]=true}
...
toggleCarryMode=function()
    if brainrotDetected then print("⚠️ Anti-Kick: Carry Speed bloqueado com pet equipado!") return end
...
toggleLaggerMode=function()
    if brainrotDetected then print("⚠️ Anti-Kick: Lagger Mode bloqueado com pet equipado!") return end
...
local function speedToggleAction()
    if brainrotDetected then print("⚠️ Anti-Kick: Ação de velocidade bloqueada (tecla Q)!") return end
    toggleCarryMode()
    saveConfig()
end
...
-- ============================================================
-- FULL CONFIG LOAD (incluindo resetKeybind, dropEnabled, batV2Enabled, espEnabled)
-- ============================================================
pcall(function()
...
local function startKL(lbl,onSet)
    cancelKL();
    KeyListen.cb=onSet; KeyListen.label=lbl; KeyListen.active=true
    ...
    task.delay(8,function()
        if KeyListen.label==cap and KeyListen.active then
            ...
            if lbl and lbl.Parent then
                lbl.Text=prettyKey(Keys.guiHide);
...
UIS.InputBegan:Connect(function(inp,gp)
    ...
    if lb and lb.Parent then
        lb.Text=prettyKey(k);
        lb.BackgroundColor3=C.blue; lb.BackgroundTransparency=0.5
    end
...
local function addInputRow(parent,label,value,order,cb)
    ...
    Box.FocusLost:Connect(function()
        ...
        if n and n>0 then cb(n) else Box.Text=tostring(value) end
    end)
...
;(function()
    ...
    if spBtn then
        spBtn.TextColor3=C.white; spBtn.BackgroundTransparency=0.2;
        local i2=spBtn:FindFirstChild("indicator");
        if i2 then i2.BackgroundTransparency=0.3 end
    end
end)()
...
;(function()
    ...
    -- ESP TOGGLE (visual)
    -- ============================================================
    addSectLbl(vi, "ESP (quadrado + linha)", 17)
    local _, espSetVisual = addToggleRow(vi, "ESP (preto)", espEnabled, 18, nil, function(on)
        if on then startESP() else stopESP() end
        saveConfig()
    end)
    espSetVisual = espSetVisual
    if espEnabled then espSetVisual(true) end
end)()
...
-- ============================================================
-- SISTEMA ESP (quadrado + linha preta)
-- ============================================================
local espEnabled = false
local espObjects = {}
local espConn = nil
local espSetVisual = nil

local function clearESPObjects()
    for _, objs in pairs(espObjects) do
        if objs.square and objs.square.Remove then objs.square:Remove() end
        if objs.line and objs.line.Remove then objs.line:Remove() end
    end
    espObjects = {}
end
...
local function createESPObjects()
    for _, plr in ipairs(Players:GetPlayers()) do
        if plr ~= LP and plr.Character and plr.Character:FindFirstChild("HumanoidRootPart") then
            ...
            ln.Color = Color3.fromRGB(0,0,0)
            ln.Thickness = 2
            ln.Visible = true

            espObjects[plr] = { square = sq, line = ln }
        end
    end
end
...
local function updateESP()
    ...
    for plr, objs in pairs(espObjects) do
        ...
        if not char then
            if objs.square then objs.square.Visible = false end
        ...
        local size = math.clamp(150 / dist, 10, 80)

        local sq = objs.square
        sq.Position = Vector2.new(screenPos.X - size/2, screenPos.Y - size/2)
        sq.Size = Vector2.new(size, size)
        sq.Visible = true
...
function toggleESP()
    ...
    if espSetVisual then espSetVisual(espEnabled) end
    saveConfig()
    return espEnabled
end

-- ============================================================
-- UNWALK (caso não tenha sido definido)
-- ============================================================
...
function stopUnwalk()
    ...
    if char then
        ...
        if unwalkSavedAnimate then
            local newAnim = unwalkSavedAnimate:Clone()
            newAnim.Parent = char
...
-- INÍCIO DO SCRIPT
-- ============================================================
print("✅ ASTA REMAKE LOADED – Anti-Kick 100% robusto + Bat V2 (tecla B) + ESP preto")
